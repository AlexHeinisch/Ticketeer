...
def get_users_by_search(service: UserService):
    req: UserSearchRequest = UserSearchRequestSchema().load(request.args)
...
def post_user(service: UserService):
    usr : User = UserSchema().load(request.get_json())
...
def patch_user(service: UserService, name: str):
    req : UserUpdateRequest = UserUpdateRequestSchema().load(request.get_json())
    req.username = name
